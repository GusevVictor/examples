...
# Решение
reverse_timer = int(input('Укажите ожидаемое время приготовления (сек): '))
is_stop = 0
seconds = 0

for time in range((reverse_timer - 1), 0, -1):
  print(f'Осталось {time} секунд')
  seconds += 1
  is_stop = int(input('Вы хотите остановить разогрев? (0 - нет, 1 - да): '))
  if is_stop:
    print(f'Ваша еда готова, можете забрать! Прошло {seconds} секунд')
    break
else:
