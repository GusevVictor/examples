...
# Решение
def calculate_danger_level(x):
  return x**3 - 3 * x**2 - 12 * x + 10


minimum_depth, maximum_depth = 0.0, 4.0
danger_level = float(
  input('Введите максимально допустимый уровень опасности: '))

while maximum_depth - minimum_depth >= danger_level:
  half_division_result = (minimum_depth + maximum_depth) / 2
  if calculate_danger_level(minimum_depth) * calculate_danger_level(
      half_division_result) < 0:
    maximum_depth = half_division_result
  else:
    minimum_depth = half_division_result
result = (minimum_depth + maximum_depth) / 2
